...
def sample_occ(age):
    if age <= 17:
        ...
        cats = ['就業','退休','家管','其他/待業'];   w = [0.12, 0.70, 0.15, 0.03]

    pw = np.array(w, dtype=float); pw /= pw.sum()
    status = rng.choice(cats, p=pw)
    if status == '就業':
        return rng.choice(OCC_CATS, p=TAIPEI_OCC_W)
    return status
...
# ── 7. 政黨傾向（113_partyXarea.xlsx 各區得票）───────────────────────────────
df_party = pd.read_excel(BASE_TP + "113_partyXarea.xlsx", sheet_name="臺北市", header=None)

PARTY_CATS = ['民進黨','國民黨','台灣民眾黨','其他政黨']

party_weights = {}   # {district: np.array(4)}

for idx in range(6, len(df_party)):
    row      = df_party.iloc[idx]
    dist_raw = str(row[0]).strip().replace('　','').replace(' ','')
    vil      = row[1]
    if dist_raw in DISTRICTS and (pd.isna(vil) or str(vil).strip() == 'nan'):
        valid = _safe_int(row[19])
        if valid > 0:
            dpp = _safe_int(row[8])
            kmt = _safe_int(row[11])
            tpp = _safe_int(row[14])
            oth = valid - dpp - kmt - tpp
            w   = np.array([dpp, kmt, tpp, max(oth, 0)], dtype=float)
            party_weights[dist_raw] = w / w.sum()

# Fallback: use Taipei city-wide total (row 5)
_tr  = df_party.iloc[5]
_v   = _safe_int(_tr[19])
_w   = np.array([_safe_int(_tr[8]), _safe_int(_tr[11]), _safe_int(_tr[14]),
                 max(_v - _safe_int(_tr[8]) - _safe_int(_tr[11]) - _safe_int(_tr[14]), 0)], dtype=float)
_city_party_w = _w / _w.sum()
...
for pid in range(1, n_total + 1):
    ...
    # 教育程度
    edu_w = edu_by_dist.get(dist, taipei_edu)
...
print('\n=== 政黨傾向 ===')
tp_party_ref = {'民進黨': 0.334, '國民黨': 0.375, '台灣民眾黨': 0.218, '其他政黨': 0.073}
...
# ── 11. 儲存 Excel ─────────────────────────────────────────────────────────
out_path = BASE + "taipei_personas_3000.xlsx"
